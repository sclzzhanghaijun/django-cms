...
from common.conf import settings as uedsetings
import os
import json
...
def get_ueditor_settings(request):
    json_data = json.dumps(
        uedsetings.UEditorUploadSettings,
        ensure_ascii=False)
...
def get_ueditor_controller(request):
    return_info = {
        ...
        'original': '',
        'type': '',
        'state': '请先登录',  # 上传状态，成功时返回SUCCESS,其他任何值将原样返回至图片上传框中
        'size': 0
    }
...
def list_files(request):
    ...
    allowFiles = {
        "listfile": uedsetings.UEditorUploadSettings.get("fileManagerAllowFiles", []),
        "listimage": uedsetings.UEditorUploadSettings.get("imageManagerAllowFiles", [])
    }
    listSize = {
        "listfile": uedsetings.UEditorUploadSettings.get("fileManagerListSize", ""),
        "listimage": uedsetings.UEditorUploadSettings.get("imageManagerListSize", "")
    }
    listpath = {
        "listfile": uedsetings.UEditorUploadSettings.get("fileManagerListPath", ""),
        "listimage": uedsetings.UEditorUploadSettings.get("imageManagerListPath", "")
    }
    # 取得参数
    ...
    root_path = os.path.join(
        uedsetings.gSettings.MEDIA_ROOT, listpath[action]).replace("\\", "/")
    ...
    for item in items:
        ...
        if os.path.isdir(item_fullname):
            ...
            if is_allow_list:
                files.append({
                    "url": urljoin(
                        uedsetings.gSettings.MEDIA_URL,
                        os.path.join(
...
def UploadFile(request):
    ...
    try:
        ...
        UploadFieldName = request.GET.get(
            upload_field_name[action], uedsetings.UEditorUploadSettings.get(action, "upfile")
        )
        ...
        if action in upload_allow_type:
            allow_type = list(request.GET.get(upload_allow_type[
                                                  action],
                                              uedsetings.UEditorUploadSettings.get(upload_allow_type[action], "")))
        ...
        max_size = long(request.GET.get(upload_max_size[
                                            action], uedsetings.UEditorUploadSettings.get(upload_max_size[action], 0)))
        if max_size != 0:
            from common.utils import FileSize
        ...
        # 取得输出文件的路径
        OutputPathFormat, OutputPath, OutputFile, OutUrl = get_output_path(
        ...
        if state == "SUCCESS":
            if action == "uploadscrawl":
                ...
                upload_module_name = uedsetings.UEditorUploadSettings.get(
                    "upload_module", None)
...
def catcher_remote_image(request):
    ...
    allow_type = list(request.GET.get(
        "catcherAllowFiles", uedsetings.UEditorUploadSettings.get("catcherAllowFiles", "")))
    max_size = long(request.GET.get(
        "catcherMaxSize", uedsetings.UEditorUploadSettings.get("catcherMaxSize", 0)))
    ...
    for remote_url in remote_urls:
        ...
        if remote_original_ext in allow_type:
            ...
            catcher_infos.append({
                "state": state,
                "url": urljoin(uedsetings.gSettings.MEDIA_URL, o_path_format),
...
def get_output_path(request, path_format, path_format_var):
    # 取得输出文件的路径
    OutputPathFormat = (request.GET.get(path_format, uedsetings.UEditorSettings[
        "defaultPathFormat"]) % path_format_var).replace("\\", "/")

    # 分解OutputPathFormat
    OutputPath, OutputFile = os.path.split(OutputPathFormat)

    OutputPath = os.path.join(uedsetings.gSettings.MEDIA_ROOT + '/' + path_format_var['date'], OutputPath)

    # 如果OutputFile为空说明传入的OutputPathFormat没有包含文件名，因此需要用默认的文件名
    #if not OutputFile:
    OutputFile = uedsetings.UEditorSettings[
    ...
    OutUrl = uedsetings.gSettings.MEDIA_URL + path_format_var['date'] + '/' + OutputFile
    return (OutputPathFormat, OutputPath, OutputFile, OutUrl)
...
def save_scrawl_file(request, filename):
    ...
    try:
        content = request.POST.get(
            uedsetings.UEditorUploadSettings.get("scrawlFieldName", "upfile"))
        f = open(filename, 'wb')
